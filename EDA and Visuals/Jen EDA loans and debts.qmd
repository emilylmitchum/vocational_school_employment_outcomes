...
layout(title = "Average Tuition and Debt vs. Enrollment from 2019 to 2023", #add plot title and axis 
           barmode = "overlay",
          xaxis = list(title = "Year"),
...
Shapiro shows less than 0.05, indicating no deviation from normality. Normality assumptions not satisfied for all groups.
...
ggplot(joined_loan_enroll_factor, aes(x= Average_loan_factor, y = Average_enroll , fill = Average_loan_number_factor)) + 
geom_boxplot() +
...
#shapiro-wilk test for each group 
shapiro.test(joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_factor == "Low Loan Amount"])

shapiro.test(joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_factor == "High Loan Amount"])

shapiro.test(joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_number_factor == "Low Loan Count"])

shapiro.test(joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_number_factor == "High Loan Count"])
```

data: joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_factor == "Low Loan Amount"\] shapiro.test(joined_loan_enroll_factor\$Average_enrW = 0.78708, p-value = 0.08099

data: joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_factor == "High Loan Amount"\] W = 0.72916, p-value = 0.01236

data: joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_number_factor == "Low Loan Count"\] W = 0.78708, p-value = 0.08099

data: joined_loan_enroll_factor$Average_enroll[joined_loan_enroll_factor$Average_loan_number_factor == "High Loan Count"\] W = 0.72916, p-value = 0.01236

Shapiro shows some p-values greater than 0.05, indicating deviation from normality. Normality assumptions satisfied for some groups like the low level number of loan recipients and low loan amount.

```{r}
#homogeneity of variance - Levene's test 
leveneTest(Average_enroll ~ Average_loan_factor, data = joined_loan_enroll_factor)

leveneTest(Average_enroll ~ Average_loan_number_factor, data = joined_loan_enroll_factor)
```

Levene's Test for Homogeneity of Variance (center = median) Df F value Pr(\>F) group 1 0.0174 0.8982 8
...
```{r}
#two way anova 
loan_enroll_fit <- aov(Average_enroll ~ Average_loan_factor * Average_loan_number_facto, data = joined_loan_enroll_factor)
summary(loan_enroll_fit)
```

Df Sum Sq Mean Sq F value Pr(\>F) Average_loan_factor 1 8.623e+07 8.623e+07 0.023 0.884 Residuals 8 3.056e+10 3.820e+09
...
```{r}
#summarize debt enrollment map by states 
enrollment_debt_map2 <- enrollment_debt_map %>%
    group_by(state) %>%
    summarize(Avg_percent_trade_school = mean(percent_trade_school),
             Avg_debt_amount = mean(Total_Debt_Amount))

#create factor for average debt amount 
enrollment_debt_map2$Avg_debt_amount_factor <- ifelse(enrollment_debt_map2$Avg_debt_amount < mean(enrollment_debt_map2$Avg_debt_amount), "Low debt amount", "High debt amount")

```

```{r}
#create boxplot 
ggplot(enrollment_debt_map2, aes(x= Avg_debt_amount_factor, y = Avg_percent_trade_school)) + 
geom_boxplot() +
labs(title = "Distribution of Debts and Enrollments",
    x = "Debt Amount",
    y = "Enrollment")

#save boxplot 
ggsave("EDA and Visuals/debt_enroll_map_boxplot.png")
```

```{r}
#make sure debt factor is factor 
enrollment_debt_map2$Avg_debt_amount_factor <- as.factor(enrollment_debt_map2$Avg_debt_amount_factor)
enrollment_debt_map2$state <- as.factor(enrollment_debt_map2$state)

#shapiro wilk tests 
shapiro.test(enrollment_debt_map2$Avg_percent_trade_school[enrollment_debt_map2$Avg_debt_amount_factor == "Low debt amount"])

shapiro.test(enrollment_debt_map2$Avg_percent_trade_school[enrollment_debt_map2$Avg_debt_amount_factor == "High debt amount"])

#Levene's test 
leveneTest(Avg_percent_trade_school ~ Avg_debt_amount_factor, data = enrollment_debt_map2)
#leveneTest(Avg_percent_trade_school ~ state, data = enrollment_debt_map2)

#statistical test of debt and enrollment by state 
#one way anova 
debt_enroll_state_fit <- aov(Avg_percent_trade_school ~ Avg_debt_amount_factor,data = enrollment_debt_map2)
summary(debt_enroll_state_fit)
```

> summary(debt_enroll_state_fit) Df Sum Sq Mean Sq F value Pr(\>F) Avg_debt_amount_factor 1 329 328.7 1.41 0.241 Residuals 48 11193 233.2

One- way anova: p-value not significant. Failed to reject null hypothesis